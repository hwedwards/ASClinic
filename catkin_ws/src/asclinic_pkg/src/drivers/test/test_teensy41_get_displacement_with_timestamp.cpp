	...
	test_config_t *test_config)
{
	...
	drive_one_wheel(pololu_smc2, speed2 * 0.99); // compensating for the difference in speed
...
event_log_t *allocate_memory_to_log_data(test_config_t *test_config)
{
	...
	int array_size = (int)((test_config->duration_micro_seconds + 1000000) / (double)test_config->polling_delta_t);
	// printf("Array size = %d\n", array_size);
...
std::string generate_log_file_name()
{
	...
	std::string logfile_base ("log/log_teensy41_get_displacement_with_timestamp");
