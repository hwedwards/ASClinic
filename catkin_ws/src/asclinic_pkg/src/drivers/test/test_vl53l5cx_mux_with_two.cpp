...
int main()
{
	...
	result = vl53l5cx_object_b.initialise_and_start_ranging(resolution, ranging_frequency_hz, integration_time_ms, sharpener_percent, target_order, ranging_mode);
	if (result)
	{
		printf("VL53L5CX - sensor initialised and ranging started for mux channel %d, for I2C address %d\n", vl53l5cx_object_a.get_mux_channel(), vl53l5cx_object_a.get_i2c_address() );
	}
	...
	while (true)
	{
		...
		uint16_t interval_between_checks_usec = 100;
		uint16_t number_of_checks = 10;

		// Read data from the VL53L5CX distance sensor
		VL53L5CX_ResultsData tof_result_a;
		bool success_get_ranging_data_a = vl53l5cx_object_a.check_data_ready_multiple_then_get_distance_measurement(&tof_result_a, interval_between_checks_usec, number_of_checks);

		VL53L5CX_ResultsData tof_result_b;
		bool success_get_ranging_data_b = vl53l5cx_object_b.check_data_ready_multiple_then_get_distance_measurement(&tof_result_b, interval_between_checks_usec, number_of_checks);

		// If a result was successfully retrieved:
		if (success_get_ranging_data_a && success_get_ranging_data_b)
		{
			printf("Get results was successful.");
			printf("\"silicon_temp_degc (A,B)\" = (%5d,%5d)\n", tof_result_a.silicon_temp_degc, tof_result_b.silicon_temp_degc);

			printf("\"target_status\" and \"distance_mm\" for A and B =\n");
			for (int i_row=0; i_row<4; i_row++)
			{
				for (int i_col=0; i_col<4; i_col++)
					printf("%4d", tof_result_a.target_status[i_row*4+i_col]);
				printf("    ");
				for (int i_col=0; i_col<4; i_col++)
					printf("%5d", tof_result_a.distance_mm[i_row*4+i_col]);
				printf("    ");
				for (int i_col=0; i_col<4; i_col++)
					printf("%5d", tof_result_b.distance_mm[i_row*4+i_col]);
				printf("    ");
				for (int i_col=0; i_col<4; i_col++)
					printf("%4d", tof_result_b.target_status[i_row*4+i_col]);
				printf("\n");
			}
		}
		else
